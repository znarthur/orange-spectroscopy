...
class Testals(unittest.TestCase):
    def test_als_Basic(self):
        data = Table.from_numpy(None, [[1.0, 2.0, 10.0, 5.0],
                                       [3.0, 5.0, 9.0, 4.0]])
        method = ALSP(als_type=0, lam=float(100E+6), pals=0.5, itermax=100)
        process = method(data)
    ...
    def test_arpls_basic(self):
        data = Table.from_numpy(None, [[1.0, 2.0, 10.0, 5.0],
                                       [3.0, 5.0, 9.0, 4.0]])
        method = ARPLS(als_type=1, lam=float(100E+5), ratioarpls=0.5, itermax=100)
        process = method(data)
        check = Table.from_numpy(None, [[-0.5, -1.5, 4.5, -2.5],
    ...
    def test_airpls_basic(self):
        data = Table.from_numpy(None, [[1.0, 2.0, 10.0, 5.0],
                                       [3.0, 5.0, 9.0, 4.0]])
        method = AIRPLS(als_type=2, lam=100, itermax=10, porderairpls=1)
        process = method(data)
